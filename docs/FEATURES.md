...
| Airtime and outbound delivery | Durable priority queue with commit-safe publication, current dispatch eligibility after waits, serialized egress, completion-based conservative airtime/retry accounting, recovered pacing, emergency reserve, quiet hours, dedupe, receipts, and delivery-health trends. | Automated-tested | unreleased @ `HEAD` (2026-09-07); introduced `3d65058` |
...
| BBS, mail, and operator inbox | Boards, threads, posts, subscriptions, digests, transactional local mail and replies with recovery of legacy interrupted sends, moderation, and one replyable operations inbox. | Automated-tested | unreleased @ `HEAD` (2026-09-05); introduced `41a6c85` |
| Community Watch | Transactional incident intake, authorized public-location corrections, reactions, operator changes and expiry with permanent local reference bindings; explicit local responsibility and accepted handoffs through version-bound web/verified-DM workflows; cross-Outpost origin/provenance reconciliation, human merge/unmerge, delivery-accountable and proximity-targeted responder alerts, member-relative mesh listings, retrying zero-recipient escalation, per-stage bounded repeats, all-clear, and maps. | Simulated | unreleased @ `HEAD` (2026-09-07); introduced `8323143` |
...
| NOAA SAME / RTL-SDR | Supervised receive-only rtl_fm/samedec pipeline, distinct fresh audio/decode and dated station evidence, drill labeling, county/review gates, CAP dedupe and bounded restart. | Hardware-gated | unreleased @ `HEAD` (2026-09-09); introduced `764516a` |
...
| Local AI assistant | ASK runtime with permission-scoped and revision-checked retrieval, conservative cited-record grounding, deterministic fallback, reserved non-AI worker capacity, frozen synthetic comparative evaluation and blinded review export, review console, native HailoRT Qwen3-VL provider, bounded device reacquisition, and explicit readiness state. | Hardware-gated | unreleased @ `HEAD` (2026-08-27); introduced `6b3d01a` |
| Installation and diagnostics | Resumable field checklist, GPS-assisted regional map setup and bounded overview downloads with verified USB import/export, mDNS and expiring isolated hotspot access, distinct wallboard accounts, live redacted diagnostics, 18 scoped readiness checks with pass/fail/stale/unknown/operator-attested evidence, independent static boot-schema inspection, authenticated releases, explicit forward recovery from incompatible boot releases, installed-store ownership guards, health-gated Hailo handoff, and hardened service tooling. Read-only OS/RTC time diagnostics distinguish synchronization, bounded process holdover and clock steps; uncertain time gates destructive retention and timestamp-sensitive work. A startup correction before any trusted native or peer time recovers automatically after two fresh stable OS synchronization checks. | Automated-tested | unreleased @ `HEAD` (2026-09-09); introduced `d614d85` |
...
- Automated: [tests/integration/test_governor_eligibility.py::test_exhausted_class_does_not_starve_another_eligible_class](../tests/integration/test_governor_eligibility.py) — All thirty distinct exhausted/eligible class pairs dispatch eligible work with the production durable outbox, preserving budget and attempt accounting.
- Automated: [tests/integration/test_governor_eligibility.py::test_smaller_eligible_item_can_pass_without_reordering_deferred_items](../tests/integration/test_governor_eligibility.py) — Smaller eligible packets can pass temporarily budget-blocked work in each class without reordering deferred items.
- Automated: [tests/integration/test_governor_eligibility.py::test_restart_keeps_class_eligibility_and_expired_work_has_no_false_send](../tests/integration/test_governor_eligibility.py) — Recovered airtime history still permits eligible replies, while expired work remains terminal without a false send or attempt.
- Automated: [tests/integration/test_outbox_commit_publication.py::test_supersession_is_invisible_until_commit_and_rollback_preserves_original](../tests/integration/test_outbox_commit_publication.py) — Real durable queue supersession stays invisible before commit, and rollback preserves previously queued work without phantom admission metrics.
- Automated: [tests/integration/test_outbox_commit_publication.py::test_repeated_cancellation_settles_commit_and_publication_before_next_writer](../tests/integration/test_outbox_commit_publication.py) — Repeated cancellation cannot release writer ownership before commit and synchronous publication settle.
- Automated: [tests/integration/test_outbox_commit_publication.py::test_checkin_commit_cancellation_keeps_solicitations_and_held_work_for_recovery](../tests/integration/test_outbox_commit_publication.py) — Committed recipient and held outbox rows survive solicitation cancellation without incorrect queue retraction or duplicate solicitation.
- Automated: [tests/integration/test_outbox_commit_publication.py::test_test_owned_process_kill_recovers_only_committed_queue_and_domain_work](../tests/integration/test_outbox_commit_publication.py) — Test-owned SIGKILL before/after commit and temporary-store restart preserve the committed domain/outbox boundary without physical-radio claims.
- Operations: [docs/OUTBOX-COMMIT-PUBLICATION.md](OUTBOX-COMMIT-PUBLICATION.md) — Post-commit local publication, ownership/cancellation, fail-closed egress, quiesced recovery, compatibility and remaining incident integration limits.
- Automated: [tests/integration/test_governor_timing.py::test_deadline_passed_during_await_cannot_start_radio_work](../tests/integration/test_governor_timing.py) — Current expiry rejects work delayed across telemetry, observer, reservation, writer and owner-validation waits without radio attempts.
- Automated: [tests/integration/test_governor_timing.py::test_policy_changes_while_waiting_preserve_pending_work](../tests/integration/test_governor_timing.py) — Current budget/share/quiet/link/power/utilisation/pacing/profile deferral preserves pending work without consuming an attempt.
- Automated: [tests/integration/test_governor_timing.py::test_slow_radio_accounting_uses_completion_not_tick_start](../tests/integration/test_governor_timing.py) — Slow success, failure and cancellation retain conservative airtime through recovery and backward wall-clock steps.
- Automated: [tests/integration/test_governor_timing.py::test_recovery_preserves_dispatch_gap](../tests/integration/test_governor_timing.py) — Recovered normal/multipart work cannot bypass the remaining dispatch gap.
- Operations: [docs/DISPATCH-TIMING.md](DISPATCH-TIMING.md) — Current-clock reservation and outcome boundaries, conservative accounting, index migration, compatibility and explicit physical/time-confidence limits.
...
- Automated: [tests/integration/test_mail_transactions.py::test_concurrent_sends_and_replies_keep_unique_ids_and_conversations](../tests/integration/test_mail_transactions.py) — Concurrent local sends and replies retain unique identities and shared thread context in the operator inbox.
- Automated: [tests/integration/test_mail_transactions.py::test_interrupted_send_or_reply_is_atomic_and_does_not_poison_future_mail](../tests/integration/test_mail_transactions.py) — Failures and cancellation at each write boundary roll back without blocking later sends, including after reopening the store.
- Automated: [tests/integration/test_mail_transactions.py::test_legacy_pending_mail_migration_preserves_content_routes_and_reply_links](../tests/integration/test_mail_transactions.py) — Idempotent migration repairs legacy placeholder mail while preserving content, existing conversation keys, reply links, and federation routing.
...
- Automated: [tests/integration/test_incident_transactions.py::test_concurrent_distinct_reports_have_unique_references_and_complete_origins](../tests/integration/test_incident_transactions.py) — Concurrent reports allocate unique references and commit incident, origin, and initial provenance together.
- Automated: [tests/integration/test_incident_responsibility.py::test_concurrent_offers_and_replayed_accept_do_not_replace_newer_owner](../tests/integration/test_incident_responsibility.py) — Real writer transactions separate offers from accepted responsibility and reject competing/replayed decisions without overwriting the newer owner.
- Automated: [tests/integration/test_responsibility_mesh.py::test_handheld_private_task_transcript_crosses_real_intake_and_governed_replies](../tests/integration/test_responsibility_mesh.py) — Verified private TASK intake and TUI choices cannot become public pending-position/emergency reports; all bounded replies use the normal governor.
- Automated: [tests/browser/test_responsibility_browser.py::test_phone_desktop_offer_accept_handoff_and_no_xss](../tests/browser/test_responsibility_browser.py) — Actual authenticated ASGI/Chromium forms support explicit offers, team/account acceptance and completion at 320px/desktop widths, without HTML execution.
- Operations: [docs/INCIDENT-RESPONSIBILITY.md](INCIDENT-RESPONSIBILITY.md) — Local authority, decision semantics, bounded workflows, reviewed identities, time/restart/restore limits, privacy and physical-qualification boundaries.
- Automated: [tests/integration/test_incident_change_events.py::test_changes_coalesce_atomically_without_radio_or_wall_clock_order](../tests/integration/test_incident_change_events.py) — Atomic source intents coalesce incident/note changes without wall-clock ordering or a radio/delivery claim.
- Automated: [tests/integration/test_incident_change_events.py::test_interrupted_mutation_rolls_back_intent_and_recovers_after_reopen](../tests/integration/test_incident_change_events.py) — Source mutations and intents roll back together on write interruption/cancellation; reopening preserves committed state.
- Automated: [tests/integration/test_incident_change_events.py::test_upgrade_backfills_current_heads_once_and_full_backup_preserves_intents](../tests/integration/test_incident_change_events.py) — Upgrade seeds current heads once and full SQLite backup/reopen preserves pending metadata and lineage.
- Automated: [tests/integration/test_incident_transactions.py::test_concurrent_reports_through_real_router_and_authenticated_web](../tests/integration/test_incident_transactions.py) — Production-wired guest radio reports and authenticated web submissions survive simultaneous intake.
- Automated: [tests/integration/test_incident_transactions.py::test_creation_rolls_back_at_every_write_boundary_and_recovers_after_reopen](../tests/integration/test_incident_transactions.py) — Write failures and actual task cancellation roll back partial intake and allow subsequent reports after closing and reopening the store.
- Automated: [tests/integration/test_incident_transactions.py::test_incident_mutations_roll_back_history_counts_and_provenance_together](../tests/integration/test_incident_transactions.py) — Reactions, operator updates/corrections, and expiry roll back state and provenance at every write boundary.
- Automated: [tests/integration/test_incident_transactions.py::test_concurrent_reactions_and_operator_updates_have_atomic_counts_and_sequences](../tests/integration/test_incident_transactions.py) — Concurrent member reactions and operator notes preserve unique sequences, idempotent reaction counts, and monitoring status.
- Automated: [tests/integration/test_transactions.py::test_cancelled_begin_rolls_back_before_releasing_the_writer](../tests/integration/test_transactions.py) — Cancellation while beginning a transaction rolls back before another request acquires the writer.
- Automated: [tests/integration/test_incident_references.py::test_partition_delayed_commands_cannot_target_a_new_incident](../tests/integration/test_incident_references.py) — Delayed INC/CONFIRM/DISPUTE commands resolve retained originals or explicitly fail after deletion, while reaction acknowledgements name the affected report and origin.
- Automated: [tests/integration/test_incident_references.py::test_migration_retires_ambiguous_legacy_numbers_without_changing_incident_identity](../tests/integration/test_incident_references.py) — Migration retains unique legacy numbers and retires reused ambiguous numbers without changing incident identity or content.
- Automated: [tests/integration/test_incident_references.py::test_import_merge_unmerge_and_reimport_keep_reference_identity](../tests/integration/test_incident_references.py) — Federated import, human merge/unmerge, and reimport of a purged origin preserve local reference identity.
- Automated: [tests/integration/test_incident_location.py::test_handheld_follows_exact_ack_then_corrects_without_cached_gps](../tests/integration/test_incident_location.py) — The actual missing-location ACK completes through an authenticated reporter reply without cached GPS attachment.
- Automated: [tests/integration/test_incident_location.py::test_correction_rolls_back_each_write_and_recovers_after_reopen](../tests/integration/test_incident_location.py) — Location state, update history, origin metadata, provenance, and actor audit roll back at all five write boundaries.
- Automated: [tests/integration/test_incident_location.py::test_corrected_location_federates_same_uid_and_retains_peer_boundary](../tests/integration/test_incident_location.py) — Same-second corrections and coordinate withholding retain identity through export/quarantine/import; geographic policy remains enforced.
- Automated: [tests/browser/test_mobile_navigation.py::test_watch_incident_intake_is_functional_and_browser_clean](../tests/browser/test_mobile_navigation.py) — Unlocated intake, explicit operator correction, map marker, and subsequent incident actions on phone/desktop in all three themes.
...
- Automated: [tests/integration/test_same_reception.py::test_repeated_process_loss_recovers_without_duplicate_actionable_warnings](../tests/integration/test_same_reception.py) — Repeated real subprocess receiver/decoder failures recover with deduplicated events, non-broadcastable drills and preserved approval gates.
- Automated: [tests/browser/test_same_reception_ui.py::test_operator_can_distinguish_audio_drill_decode_and_station_observation](../tests/browser/test_same_reception_ui.py) — Authenticated production API and Chromium show distinct evidence at phone/desktop widths in all themes, independently of forecast availability.
- Operations: [docs/SDR-RECEPTION.md](SDR-RECEPTION.md) — Dated station/antenna and legitimate broadcast-test procedure, freshness limits, pinned offline fixture and interruption witnesses.

Known limitations:

- Intelligible station/antenna qualification and receipt of a legitimate broadcast test remain physical gates; PCM level and recorded-fixture decoding cannot certify them.
- Decoder evidence is bounded, context-bound and current-process only; older durable message records do not establish present reception. Operator station observations remain finite statements, not automatic certification.
...
- Automated: [tests/integration/test_commit_durability.py::test_full_writer_policy_survives_fresh_migrations_and_reopen](../tests/integration/test_commit_durability.py) — The actual serialized writer uses FULL after fresh migrations and ordinary reopen; another connection's PRAGMA is not used as writer evidence.
- Automated: [tests/integration/test_commit_durability.py::test_weaker_writer_policy_fails_closed_and_releases_connection](../tests/integration/test_commit_durability.py) — Weakened writer configuration before or after migrations fails startup and releases its connection.
- Operations: [docs/COMMIT-DURABILITY.md](COMMIT-DURABILITY.md) — Acknowledged-record commit contract, reproducible disposable-store benchmarks and separate physical storage/power acceptance.
- Automated: [tests/integration/test_encrypted_recovery.py::test_memory_snapshot_and_complete_fenced_restore](../tests/integration/test_encrypted_recovery.py) — Actual encrypted round trip retains private records, identity and revision evidence; a fresh app and repeated restart remain fenced, with named-operator access and no RF.
...
- Automated: [tests/unit/test_offline_kit.py::test_actual_offline_install_uses_only_verified_copy_and_never_activates](../tests/unit/test_offline_kit.py) — Synthetic wheel fixture installs into a fresh inactive venv from an independently copied hash-verified kit after removing source/wheelhouse access; checks exact package inventory and ignores ambient index/cache/target configuration. Actual appliance and second-operator qualification remain open.
- Operations: [docs/OFFLINE-REPLACEMENT.md](OFFLINE-REPLACEMENT.md) — Runtime kit preparation/verification, explicit boot/vendor/client/map/licensing inventory, permanent powered operator LAN and second-operator offline recovery/expiry/reboot record.
...
- Operations: [docs/FEDERATION-REVIEW-SAFETY.md](FEDERATION-REVIEW-SAFETY.md) — Review transaction ownership, API/handheld compatibility, deployment precautions, automatic-policy distinction and evidence limits.
- Automated: [tests/integration/test_federation_bundles.py::test_radio_off_restart_preview_atomic_import_and_later_revision_reconciliation](../tests/integration/test_federation_bundles.py) — Two temporary Outposts import a signed page without RF, retain exact radio-reconciliation digests and later converge a changed producer revision without duplicate incidents.
- Automated: [tests/browser/test_bundle_browser.py::test_actual_browser_signed_download_preview_upload_commit_is_offline_and_safe](../tests/browser/test_bundle_browser.py) — Actual authenticated Chromium download/upload and explicit privacy-bound review at phone/desktop widths, with external requests blocked and no requested RF action.
- Operations: [docs/FEDERATION-BUNDLES.md](FEDERATION-BUNDLES.md) — Commissioning and existing trust prerequisites, signed plaintext schema, confidentiality scopes, atomic/replay/key/time contract, capacity limits and physical-evidence boundaries.
- Automated: [tests/integration/test_domain_adapters.py::test_transient_dispatch_adapters_share_reassembly_counters_and_receipt_owner](../tests/integration/test_domain_adapters.py) — Extracted ingress borrows actual application owners across multipart frames; remote storage and governed receipt admission do not imply import or RF completion.
- Automated: [tests/integration/test_domain_adapters.py::test_pairing_handshake_crosses_dispatch_and_governed_replies_without_duplicate_owners](../tests/integration/test_domain_adapters.py) — Both simulated Outposts complete the existing authenticated pairing handshake through the extracted coordinator and governed queue, without creating parallel protocol state.
- Operations: [docs/TRANSACTION-OWNERSHIP.md](TRANSACTION-OWNERSHIP.md) — Current domain owners, behavior-preserving ingress/route extraction, source-equivalence checks, and remaining physical qualification boundaries.
- Automated: [tests/integration/test_federation_incident_events.py::test_event_crosses_real_framing_and_governor_without_import_or_human_ack](../tests/integration/test_federation_incident_events.py) — Negotiated incident ingress commits quarantine before a typed exact-revision/content receipt traverses the governed simulated radio; it never imports or acts for responders.
- Automated: [tests/integration/test_federation_incident_events.py::test_lost_receipt_retry_survives_reopen_and_preserves_review](../tests/integration/test_federation_incident_events.py) — Fresh-counter identical retries recover exact storage receipts after reopen without resetting pending, imported or rejected review state.
- Operations: [docs/FEDERATION-INCIDENT-EVENTS.md](FEDERATION-INCIDENT-EVENTS.md) — Event capability, target/counter/policy checks, receipt digest contract, durable quota limits, parent-first notes and sender/field-work boundaries.
- Automated: [tests/integration/test_incident_sender_handoff.py::test_handoff_binds_versions_and_note_parent_without_consuming_or_sending](../tests/integration/test_incident_sender_handoff.py) — Bounded per-peer staging binds exact source versions/content and note parent identity without consuming the source journal, admitting radio work or implying remote acceptance.
- Automated: [tests/integration/test_incident_sender_handoff.py::test_killed_handoff_process_recovers_without_skipping_source_work](../tests/integration/test_incident_sender_handoff.py) — Test-owned SIGKILL before/after handoff commit preserves source work and recovers only committed intents/checkpoints; this is not physical power-loss evidence.
- Operations: [docs/INCIDENT-SENDER-HANDOFF.md](INCIDENT-SENDER-HANDOFF.md) — Writer ownership, retained-source/independent-peer semantics, scope and lineage guards, migration, storage bounds and links to the automatic worker.
- Automated: [tests/integration/test_incident_sender.py::test_sender_to_receiver_and_exact_receipt_never_imply_human_acceptance](../tests/integration/test_incident_sender.py) — Explicit guarded admission traverses the real codec/governor and simulated peer, with exact storage receipt matching and no automatic human action.
- Automated: [tests/integration/test_incident_sender.py::test_killed_sender_recovers_atomic_counter_association_and_frames](../tests/integration/test_incident_sender.py) — Test-owned process kills before/after commit retain counter/frame association atomically and recover guarded sending; no physical power-loss or G6 claim.
- Automated: [tests/integration/test_incident_sender.py::test_queue_deadline_is_current_after_awaited_reservation_validation](../tests/integration/test_incident_sender.py) — A deadline crossed before/during awaited authorization prevents a radio attempt despite an older governor tick timestamp.
- Automated: [tests/integration/test_incident_sender.py::test_old_authenticated_key_cannot_credit_new_key_association](../tests/integration/test_incident_sender.py) — A receipt verified before a concurrent key rotation cannot credit a new-key association; a governed reply verified with the new key succeeds.
- Automated: [tests/integration/test_incident_sender.py::test_receipt_fragments_cannot_mix_across_key_rotation](../tests/integration/test_incident_sender.py) — Partial receipts verified under different pairing keys remain separate; a complete current-key assembly is required before exact storage credit.
- Operations: [docs/INCIDENT-SENDER-ADMISSION.md](INCIDENT-SENDER-ADMISSION.md) — Explicit admission, per-attempt authorization, exact receipts, migration/privacy bounds and the separate automatic scheduling/field-acceptance contract.
- Automated: [tests/integration/test_incident_worker.py::test_change_just_after_a_worker_poll_includes_full_poll_phase_in_latency](../tests/integration/test_incident_worker.py) — Automatic production-wired sender simulations cover finite loss/reorder/restart recovery, fair bounded scheduling, expiry, cancellation and current-parent dependencies without RF or human-action claims.
- Automated: [tests/integration/test_incident_receipt_queue.py::test_identical_fresh_counter_events_share_one_pending_reply](../tests/integration/test_incident_receipt_queue.py) — Pending exact replies coalesce atomically; changed evidence/key/scope/routing and recovery fail closed, with counter/frame rollback on failure.
- Automated: [tests/integration/test_incident_delivery_api.py::test_field_report_to_reviewed_map_feed_measures_human_delay_separately](../tests/integration/test_incident_delivery_api.py) — Operator-only paged delivery stages, stale-action and audit safety, plus a REPORT-command to reviewed map-feed simulation with separately modeled human delay.
- Automated: [tests/browser/test_incident_g6.py::test_field_report_only_reaches_reviewed_browser_maps_without_wan](../tests/browser/test_incident_g6.py) — Real ASGI/Chromium, pairwise-key simulated radios, modeled application airtime, blocked WAN requests and loaded synthetic local tiles: two/three-Outpost reviewed maps within 60 seconds under the declared envelope; no review means no remote map import.
- Automated: [tests/integration/test_incident_compact.py::test_negotiation_preserves_legacy_and_exact_storage_without_human_import](../tests/integration/test_incident_compact.py) — Lossless negotiated payload-name encoding preserves exact receipts/content and legacy fallback; malformed codes, capability transitions and writer-time policy revocation fail closed.
- Automated: [tests/integration/test_incident_worker.py::test_continuous_fresh_arrivals_cannot_replenish_ahead_of_admitted_backlog](../tests/integration/test_incident_worker.py) — Higher numeric urgency is exercised on actual governor dispatch, with admitted backlog protected against continuous fresh-lane replenishment.
- Operations: [docs/INCIDENT-AUTOMATIC-DELIVERY.md](INCIDENT-AUTOMATIC-DELIVERY.md) — Automatic scheduling/receipt/control policy, simulated timing envelope, migration and the held physical G6 and remaining remote-withdrawal/retention limits.
- Automated: [tests/integration/test_federation_incident_notes.py::test_notes_require_parent_review_preserve_content_and_never_act_for_responders](../tests/integration/test_federation_incident_notes.py) — Plain notes require original-parent review, retain body and producer identity, and never change local responder state or auto-import.
- Automated: [tests/integration/test_federation_incident_notes.py::test_review_note_provenance_and_both_audits_roll_back_together](../tests/integration/test_federation_incident_notes.py) — Failures/cancellation roll back reviewed note content, provenance, revision audit, inbox decision and human audit together.
- Operations: [docs/FEDERATION-INCIDENT-NOTES.md](FEDERATION-INCIDENT-NOTES.md) — Negotiated note ownership, migration/review contract, payload limits and separation from prompt delivery and physical G6.
- Automated: [tests/integration/test_federation_item_failures.py::test_oversize_failure_crosses_real_codec_outbox_and_keeps_page_missing](../tests/integration/test_federation_item_failures.py) — Real codec and durable outbox with simulated radios distinguish oversized payloads from quarantined neighbors without false receipts or completed sync.
- Operations: [docs/FEDERATION-ITEM-FAILURES.md](FEDERATION-ITEM-FAILURES.md) — Negotiation, metadata bounds, retry/reopen semantics, stage honesty and remaining oversized-page blocking limitation.
...
- Oversized ITEM failures are visible and retryable with item_failures:1 plus reconciliation:2. Later pages can still wait behind oversized content; this is not arbitrary-size transport or a delivery guarantee (#135).
- Automatic incident delivery is software/simulation-tested, not physically qualified. Compact-capable two/three-Outpost browser cases reach reviewed maps within 60 seconds for a short report with separate five-second human review, empty queues and inactive quiet hours. Legacy storage-only examples remain 25/30 seconds, parent plus note 55/60. No arbitrary-load, firmware or physical G6 guarantee (#135).
- Fresh/backlog source pages and peer visits are bounded; per-peer intents retain a finite 30-minute window and at most three application admissions. Quiet hours, offline peers, saturation, unsupported versions and oversized content can prevent prompt delivery or expire work; cancel/retry is explicit and audited.
- Current source/key/scope/parent authority is revalidated at admission and attempt reservation. Pending exact receipt replies coalesce. Queue admission, radio completion and observed remote storage are separate; remote human review/notification/acknowledgement are not reported by this protocol.
...
- Automated: [tests/integration/test_ai_holdout.py::test_frozen_holdout_through_real_database_and_service](../tests/integration/test_ai_holdout.py) — Frozen synthetic cases exercise real retrieval, permissions and guard behavior in both guarded and deterministic modes; scripted controls do not claim model quality.
- Automated: [tests/integration/test_ai_holdout.py::test_core_commands_and_same_sender_urgent_intake_survive_ai_load](../tests/integration/test_ai_holdout.py) — Real inbound workers/router preserve board, mail and urgent intake with AI disabled, blocked and failed.
- Operations: [docs/AI-EVALUATION-2026-09-09.md](AI-EVALUATION-2026-09-09.md) — Software completion, holdout/legacy scoring separation, blinding, measurement limits and later comparison scope.
...
- Automated: [tests/unit/test_deploy_install.py::test_forward_recovery_preserves_newer_acknowledged_data_and_blocks_old_rollback](../tests/unit/test_deploy_install.py) — Explicit forward repair retains newer data and refuses incompatible manual rollback before downtime.
- Automated: [tests/integration/test_store_ownership.py::test_checkout_cannot_migrate_a_marked_installed_store](../tests/integration/test_store_ownership.py) — The real database opening path rejects checkout ownership, including a database symlink, before SQLite opens or migrates acknowledged data.
- Operations: [docs/BOOT-RECOVERY.md](BOOT-RECOVERY.md) — Backup prerequisites, verified forward repair, failure preservation, development isolation and controlled reboot evidence.
- Field: [docs/BOOT-RECOVERY-QUALIFICATION-2026-09-08.md](BOOT-RECOVERY-QUALIFICATION-2026-09-08.md) — One local Pi on source 0451303/schema 185 recovered automatically after a controlled reboot with healthy web/radio/required Hailo AI and all pre-reboot records retained, including five pending work items; power-loss and broader field gates remain separate.
...
- Automated: [tests/integration/test_radio_power.py::test_external_power_survives_adapter_governor_storage_and_readiness](../tests/integration/test_radio_power.py) — Reported external radio power survives the adapter, governor and stored history and passes its fresh readiness check without a battery percentage; missing/low readings, expiry and whole-station power remain distinct.
- Operations: [docs/RADIO-POWER.md](RADIO-POWER.md) — External-power telemetry, battery thresholds, missing/stale evidence and schema-186 compatibility without a whole-station energy claim.
- Field: [docs/RADIO-POWER-QUALIFICATION-2026-09-08.md](RADIO-POWER-QUALIFICATION-2026-09-08.md) — Local Pi on source 0dd0598/schema 186 reports external radio power with passing fresh readiness, matching served dashboard assets, healthy web/radio/native AI and all protected pre-update records retained; station energy and other field gates remain separate.
...
- Automated: [tests/integration/test_startup_time.py::test_boot_correction_recovers_queue_expiry_and_airtime_without_restart](../tests/integration/test_startup_time.py) — Delayed native startup synchronization with forward/backward six-hour and observed 259.806-second corrections recovers the production queue while preserving expiry, prior airtime and unknown-history silence; later trusted-time steps remain latched. Synthetic clocks, not a physical reboot qualification.
- Automated: [tests/browser/test_time_confidence_ui.py::test_operator_sees_startup_recovery_without_rtc_certification](../tests/browser/test_time_confidence_ui.py) — The authenticated browser's normal navigation poll refreshes a cached startup hold after clock recovery without a manual readiness run, renewed observations or RTC certification, at phone and desktop widths.
