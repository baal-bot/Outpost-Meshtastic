...
- Post-commit publication failure blocks egress until quiesced recovery; cancellation may still leave committed or uncertain work. Explicit incident admission/receipts exist, but automatic scheduling remains #135.
...
- Incident/note changes have an atomic coalesced source journal with bounded inspection; it is undispatched metadata, not per-peer queueing or delivery (#165/#135).
...
- Operations: [docs/INCIDENT-SENDER-HANDOFF.md](INCIDENT-SENDER-HANDOFF.md) — Writer ownership, retained-source/independent-peer semantics, scope and lineage guards, migration, storage bounds and remaining sender integration.
...
- Operations: [docs/INCIDENT-SENDER-ADMISSION.md](INCIDENT-SENDER-ADMISSION.md) — Explicit admission, per-attempt authorization, exact receipts, migration/privacy bounds and remaining automatic scheduling/field acceptance.
...
- Plain incident notes require incident_updates:1 plus reconciliation:2 on both peers and prior original-parent import. They are software/simulation-tested, not prompt delivery or local responder acknowledgement; oversized notes and multi-hop note relay are not qualified.
...
- incident_events:1 adds authenticated receiver-side quarantine and revision/full-content-bound storage receipts, not a sender worker or prompt delivery. Parent-first note storage, current scope, durable new-revision quotas and human review remain required; see #166/#135.
- Atomic per-peer incident staging (#167) binds current source content, policy and scan progress without consuming the source journal. It is not automatic sending; new-peer/changed-scope backlogs need scheduling and latency qualification (#135).
- Explicit incident admission (#169) atomically binds counters/frames to staged content, revalidates authority and the current deadline at attempt reservation and consumes exact storage receipts. Automatic scheduling/application retry policy, receipt-reply coalescing and G6 remain open; storage never implies review or responder acknowledgement.
