import "/nav.js";
import {byId as $, escapeHtml as safe, formatAgeSeconds, relativeAge as ago} from "/ui-primitives.js";
...
const stageRow = (stage = {after_minutes: 0, notify: "responders", channels: [3], repeat: false, proximity: "any"}) => `<div class="stage-row"><label><span>After</span><input data-stage-after type="number" min="0" value="${safe(stage.after_minutes)}"><small>minutes</small></label><label><span>Notify</span><select data-stage-notify>${["responders","trusted","all"].map(value => `<option value="${value}" ${stage.notify === value ? "selected" : ""}>${value}</option>`).join("")}</select></label><label><span>Channels</span><input data-stage-channels value="${safe((stage.channels || []).join(", "))}" placeholder="0, 3"></label><label class="repeat-control"><input data-stage-proximity type="checkbox" ${stage.proximity === "footprint" ? "checked" : ""}><span>Inside alert footprint</span></label><label class="repeat-control"><input data-stage-repeat type="checkbox" ${stage.repeat ? "checked" : ""}><span>Repeat</span></label><button data-remove-stage type="button" aria-label="Remove stage">×</button></div>`;
