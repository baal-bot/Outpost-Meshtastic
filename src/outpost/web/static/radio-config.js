  ...
  return `<section id="radio-config-card" class="ui-card panel content-panel operator-panel radio-config-card">
    ...
    <div id="radio-config-workspace" class="radio-config-workspace" hidden>
      ...
      <div class="radio-config-pane active" data-radio-pane="essentials">
        ...
        <form id="radio-lora-form" class="radio-config-form radio-config-wide"><div><p class="eyebrow">LORA</p><h3>Regional radio profile</h3><p>Region must match local law. LONG FAST, hop limit 3, and automatic transmit power are the Outpost defaults.</p></div><div class="radio-form-row"><label>Legal region<select id="radio-region" required></select></label><label>Modem preset<select id="radio-preset-config" required></select></label><label>Hop limit<input id="radio-hop-limit" type="number" min="1" max="7" required></label><label>Transmit power · dBm<input id="radio-tx-power" type="number" min="0" max="30" required><small>0 lets firmware choose.</small></label></div><label class="radio-check"><input id="radio-tx-enabled" type="checkbox"> Radio transmission enabled</label><button type="submit">Save LoRa profile</button><output id="radio-lora-result"></output></form>
      </div>
      <div class="radio-config-pane" data-radio-pane="channels" hidden>
        <form id="radio-channel-form" class="radio-config-form radio-config-wide"><div><p class="eyebrow">CHANNELS</p><h3>Meshtastic channel slots</h3><p>Active slots must be consecutive. Keys are never read back or written to Outpost logs.</p></div><div class="radio-form-row"><label>Channel slot<select id="radio-channel-index"></select></label><label>Role<select id="radio-channel-role"></select></label><label>Name<input id="radio-channel-name" maxlength="12"></label><label>Position precision<input id="radio-position-precision" type="number" min="0" max="32"><small>0 hides position; 32 is exact.</small></label></div><div id="radio-channel-policy" class="radio-policy-note"></div><fieldset><legend>Channel key</legend><label>Replace with base64 key<input id="radio-channel-psk" maxlength="44" autocomplete="off" spellcheck="false" placeholder="Leave blank to retain current key"></label><label class="radio-check"><input id="radio-channel-generate" type="checkbox"> Generate a new AES-256 key</label><small id="radio-channel-key-state"></small></fieldset><div class="radio-check-row"><label class="radio-check"><input id="radio-channel-uplink" type="checkbox"> MQTT uplink</label><label class="radio-check"><input id="radio-channel-downlink" type="checkbox"> MQTT downlink</label><label class="radio-check"><input id="radio-channel-muted" type="checkbox"> Mute received notifications</label></div><button type="submit">Save channel</button><output id="radio-channel-result"></output><div id="radio-generated-key" class="radio-generated-key" hidden><strong>Copy this key now</strong><p>It will not be shown again by Outpost.</p><code></code></div></form>
      </div>
...
export async function initRadioConfigurator({api}) {
  ...
  function render(next) {
    ...
    const badges = [
      ...
      `${state.lora.region.replaceAll("_", " ")} · ${state.lora.modem_preset.replaceAll("_", " ")}`,
  ...
  byId("radio-lora-form").addEventListener("submit", async (event) => {
    event.preventDefault();
    await apply("lora", {region: byId("radio-region").value, modem_preset: byId("radio-preset-config").value, hop_limit: intValue("radio-hop-limit"), tx_power: intValue("radio-tx-power"), tx_enabled: byId("radio-tx-enabled").checked}, "Confirm that the selected region is legal at this installation.", byId("radio-lora-result"));
  });
