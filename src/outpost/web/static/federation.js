...
async function refreshSyncStatus() {
  ...
  target.innerHTML = `<div class="transfer-summary"><span><b>${result.outbound.frames_24h}</b> mesh frames · 24h</span><span>${result.outbound.last_at ? `Last outbound ${age(result.outbound.last_at)}` : "No outbound federation traffic"}</span></div>` + (items.map(peer => {
    ...
    const success=Math.max(peer.last_sync_at||0,delivery.last_delivered_at||0,...peer.cursors.map(cursor=>cursor.updated_at||0));
    const health=peer.sync_paused?"offline":catchup.reason||security.rejected_24h||delivery.errors?"attention":delivery.pending||catchup.active?"delayed":"healthy";
