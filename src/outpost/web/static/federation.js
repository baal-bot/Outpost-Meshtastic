...
function relayHistory(item) { const events=item.history||[];if(!events.length)return "";return `<details class="relay-history"><summary>Recent custody history (${events.length})</summary>${events.map(event=>`<p><time>${new Date(event.created_at*1000).toLocaleString()}</time> · <b>${safe(event.event_kind)}</b>${event.peer_mesh_id||event.detail?.next_hop?` · ${safe(event.peer_mesh_id||event.detail.next_hop)}`:""}${event.detail?.reason?`<br>${safe(event.detail.reason)}`:""}</p>`).join("")}</details>`; }
...
async function refreshSyncStatus() {
  ...
  target.innerHTML = `<div class="transfer-summary"><span><b>${result.outbound.frames_24h}</b> mesh frames · 24h</span><span>${result.outbound.last_at ? `Last outbound ${age(result.outbound.last_at)}` : "No outbound federation traffic"}</span></div>` + (items.map(peer => {
    const transfer=peer.transfers,delivery=transfer.deliveries,security=transfer.security,catchup=transfer.catch_up,radio=transfer.paths.radio,mqtt=transfer.paths.mqtt;
    const encodingFailures=Array.isArray(transfer.encoding_failures)?transfer.encoding_failures.slice(0,8):[];
    const success=Math.max(peer.last_sync_at||0,delivery.last_delivered_at||0);
    const health=peer.sync_paused?"offline":catchup.reason||encodingFailures.length||security.rejected_24h||delivery.errors?"attention":delivery.pending||catchup.active?"delayed":"healthy";
    const state=peer.sync_paused?"offline · sync paused":catchup.reason?`sync ${safe(catchup.status||"stopped")}`:catchup.active?"catching up":health;
    const reasons=security.recent.map(item=>`${safe(item.reason)} · ${age(item.created_at)}`).join("<br>");
    const cursorLabel=catchup.cursor_mode==="producer_revision"?`producer revision ${safe(catchup.snapshot??"pending")}`:`snapshot ${new Date(catchup.snapshot*1000).toLocaleString()}`;
    const catchupStrip=peer.sync_paused?`<div class="offline-strip"><strong>Automatic federation traffic paused</strong><span>Trust is retained. A HELLO or authenticated message will resume syncing.</span></div>`:catchup.reason?`<div class="catchup-strip warn"><strong>Federation reconciliation ${safe(catchup.status||"stopped")}</strong><span>${safe(catchup.reason)} · ${safe(catchup.used||0)} / ${safe(catchup.budget||0)} items · ${safe(catchup.rounds||0)} rounds${catchup.resume_after?` · resumes ${new Date(catchup.resume_after*1000).toLocaleString()}`:""}</span></div>`:catchup.active?`<div class="catchup-strip"><strong>↻ Bounded catch-up in progress</strong><span>${catchup.waiting?"Awaiting peer response":"Next batch scheduled"} · ${safe(catchup.used||0)} / ${safe(catchup.budget||0)} items · ${cursorLabel}</span></div>`:"";
    return `<article class="transfer-card ${health}"><div class="transfer-head"><div><strong>${safe(peer.node_name || peer.mesh_id)}</strong><code>${safe(peer.mesh_id)}</code></div><span class="transfer-state">${state}</span></div><div class="path-metrics"><div><span class="path-icon radio">⌁</span><b>${radio.count_24h}</b><small>LoRa received · 24h</small><em>${radio.last_at?age(radio.last_at):"No observed traffic"}</em></div><div><span class="path-icon mqtt">◫</span><b>${mqtt.count_24h}</b><small>MQTT received · 24h</small><em>${mqtt.last_at?age(mqtt.last_at):"No observed traffic"}</em></div></div><div class="delivery-metrics"><span><b>${delivery.delivered}</b><small>Delivered</small></span><span><b>${delivery.pending}</b><small>Queued</small></span><span><b>${delivery.retries}</b><small>Retries</small></span><span><b>${delivery.recovered}</b><small>Recovered</small></span></div>${catchupStrip}<div class="security-strip ${security.rejected_24h?"warn":"clear"}"><strong>${security.rejected_24h?`⚠ ${security.rejected_24h} rejected frame${security.rejected_24h===1?"":"s"} · 24h`:"✓ No rejected frames · 24h"}</strong>${reasons?`<span>${reasons}</span>`:""}</div><div class="transfer-foot"><span>${success?`Last success ${age(success)}`:"No successful sync yet"}</span><span>Protocol counters · TX ${safe(peer.tx_counter)} · RX ${safe(peer.rx_counter)}</span><span>${peer.cursors.length} active cursor${peer.cursors.length===1?"":"s"}</span></div></article>`;
  }).join("") || `<p class="ui-empty empty">No paired sync activity yet.</p>`);
  document.querySelectorAll(".transfer-card").forEach((card,index) => {
    const transfer=items[index]?.transfers;
    for(const [label,entries] of [["Recent source encoding failures · payload not sent",transfer?.encoding_failures],["Receiving page blocked · content not received",transfer?.catch_up?.failures]]) {
      if(!Array.isArray(entries)||!entries.length)continue;
      const strip=document.createElement("div");strip.className="ui-notice item-failures";
      strip.innerHTML=`<strong>${label}</strong><span>${entries.slice(0,8).map(item=>`${safe(item.stream)} · ${safe(item.uid)} · revision ${safe(item.revision)} · ${safe(item.failure)}`).join("<br>")}</span>`;
      card.querySelector(".security-strip")?.before(strip);
    }
    const services=transfer?.services;if(!services)return;const usage=services.usage||{},open=(services.circuits||[]).filter(value=>(value.open_until||0)>Date.now()/1000).map(value=>value.service);const strip=document.createElement("div");strip.className=`service-budget-strip ${open.length?"warn":""}`;strip.innerHTML=`<strong>Peer services · ${services.permissions.length?services.permissions.map(safe).join(", "):"denied"}${open.length?` · circuit open: ${open.map(safe).join(", ")}`:""}</strong><span>${safe(usage.requests||0)} / ${safe(services.request_limit)} requests · ${Number(usage.response_airtime_seconds||0).toFixed(1)} / ${safe(services.airtime_limit_seconds)} sec airtime · ${safe(usage.denied||0)} denied</span>`;card.querySelector(".security-strip")?.before(strip);
  });
}
...
async function refreshRelayMail() { const target = $("relay-mail-history"); if (!target) return; const response = await api("/api/v1/federation/mail"); if (!response.ok) return; const result=await response.json(),items=result.items||[],usage=result.usage||[]; const budgets=usage.map(item=>`<article class="relay-mail-budget ${item.inbound_rejected?"warn":""}"><strong>${safe(item.node_name||item.mesh_id)}</strong><span>Inbound ${safe(item.inbound_accepted)} / ${safe(item.quota_mail_per_hour)} this hour · ${safe(item.inbound_rejected)} rejected</span><small>Outbound has a separate ${safe(item.quota_mail_per_hour)} / hour allowance · inbound recipient cap ${safe(item.quota_mail_per_recipient_per_hour)} / hour</small></article>`).join(""); const history=items.map(item => `<article><strong>${safe(item.direction)} · ${safe(item.state)}</strong><span>${safe(item.node_name || item.mesh_id)} → ${safe(item.recipient_handle)}</span><code>${safe(item.relay_id)}</code><time>${new Date(item.created_at*1000).toLocaleString()}</time></article>`).join("") || `<p class="ui-empty empty">No federated mail deliveries yet.</p>`; target.innerHTML=`<div class="relay-mail-usage">${budgets}</div>${history}`; }
...
async function loadStoreForward() {
  ...
  panel.innerHTML = `<div class="heading"><div><p class="eyebrow">SIGNED STORE-AND-FORWARD</p><h2>Disconnected delivery</h2></div><div><button id="rotate-relay-key" class="small-button">Rotate local key</button> <button id="refresh-relay" class="small-button">Refresh</button></div></div><p class="mqtt-note">Every hop requires an explicit peer policy. Origin signatures and idempotency survive relays; routing metadata is visible to relay operators.</p><div id="relay-summary" class="relay-summary"></div><form id="relay-create" class="relay-create"><input id="relay-destination" aria-label="Relay destination node ID" pattern="^![0-9a-fA-F]{8}$" placeholder="Destination !1234abcd" required><select id="relay-scope" aria-label="Relay content scope"><option value="request">Request</option><option value="incident">Incident</option><option value="receipt">Receipt</option></select><textarea id="relay-payload" aria-label="Relay payload JSON" maxlength="700" placeholder='Request JSON, for example {"service":"weather","args":{"lat":40.44,"lon":-79.99}}' required></textarea><label>Hop limit <input id="relay-hops" type="number" min="1" max="4" value="3"></label><button>Queue signed envelope</button><span id="relay-result"></span></form><div class="relay-columns"><section><h3>Peer relay policies</h3><div id="relay-policies"></div></section><section><h3>Origin key decisions</h3><div id="relay-origins"></div></section></div><h3>Custody queue</h3><div id="relay-queue"></div>`;
  directory.insertAdjacentElement("afterend", panel);
  $("refresh-relay").addEventListener("click", refreshStoreForward);
  $("rotate-relay-key").addEventListener("click", async () => {
    if(!await window.OutpostUI.confirm({title:"Rotate the local relay identity?",message:"Future envelopes will carry a successor proof signed by the current key. Peers that still trust the current fingerprint can adopt the new one automatically.",confirmLabel:"Rotate identity",danger:true}))return;
    const response=await api("/api/v1/federation/relay/identity/rotate",{method:"POST"}),body=await response.json();
    await window.OutpostUI.alert({title:response.ok?"Relay identity rotated":"Identity not rotated",message:response.ok?`New fingerprint: ${body.fingerprint}`:body.error.message});
...
async function refreshStoreForward() {
  ...
  const dispatchFailures=(result.queue||[]).filter(item=>!["expired","purged"].includes(item.state)&&(item.dispatch_status==="failed"||item.dispatch_status==="pending")).length;
  const terminalFailures=(result.queue||[]).filter(item=>item.state==="rejected"&&!item.dispatch_status).length;
  $("relay-summary").innerHTML = `<span><b>${active}</b> active custody items</span><span><b>${dispatchFailures}</b> local dispatch failures</span><span><b>${terminalFailures}</b> terminal transfer failures</span><span><b>${result.summary.stored_bytes}</b> stored bytes</span><span>${result.identity?.fingerprint?`Local key ${safe(result.identity.fingerprint.slice(0,16))}…`:"Local key status unavailable"}</span>`;
  if (result.summary.time_warning) { const warning=document.createElement("p");warning.className="ui-empty";warning.setAttribute("role","status");warning.textContent=result.summary.time_warning;$("relay-summary").append(warning); }
  ...
  $("relay-origins").innerHTML = result.origins.map(origin => `<article><div><strong>${safe(origin.origin_node)}</strong><small>${origin.fingerprint?`${safe(origin.fingerprint.slice(0,16))}… · ${safe(origin.state)} · from ${safe(origin.observed_from||"unavailable peer")}`:"No authoritative pin"}</small>${(origin.candidates||[]).map(candidate=>`<small>Candidate ${safe(candidate.fingerprint.slice(0,16))}… · ${safe(candidate.state)} · from ${safe(candidate.last_observed_from||"unavailable peer")} (${safe(candidate.observation_count)})</small>${candidate.state!=="trusted"||!origin.fingerprint?`<button data-origin-replace="${safe(origin.origin_node)}" data-fingerprint="${safe(candidate.fingerprint)}">Use candidate</button><button class="danger" data-origin-reject-candidate="${safe(origin.origin_node)}" data-fingerprint="${safe(candidate.fingerprint)}">Reject candidate</button>`:""}`).join("")}</div>${origin.state === "observed" ? `<button data-origin-trust="${safe(origin.origin_node)}">Trust pin</button><button class="danger" data-origin-reject="${safe(origin.origin_node)}">Reject pin</button>` : ""}${origin.fingerprint?`<button class="danger" data-origin-forget="${safe(origin.origin_node)}">Forget pin</button>`:""}</article>`).join("") || `<p class="ui-empty empty">No relayed origin keys observed.</p>`;
  queue.innerHTML = result.queue.map(item => `<article class="relay-item"><div><strong>${safe(item.scope)} · ${item.dispatch_status==="failed"?"local dispatch failed":item.dispatch_status==="pending"?"local dispatch pending":item.state==="rejected"?"terminal failure":safe(item.state)}</strong><code>${safe(item.envelope_id)}</code></div><p>${safe(item.origin_node)} → ${safe(item.destination_node)}<br>Route: ${item.route.map(safe).join(" → ")}${item.last_path ? `<br>Selected ${safe(item.last_path)} path → ${safe(item.next_hop_mesh_id)}` : ""}${item.received_transport ? `<br>Received by ${safe(item.received_transport.toUpperCase())}` : ""}<br>${safe(item.attempts)} forwarding attempt(s) · ${safe(item.dispatch_attempts||0)} local dispatch attempt(s)${item.dispatch_status?`<br>Domain outcome: ${safe(item.dispatch_status)}`:""}</p><small>Expires ${new Date(item.expires_at*1000).toLocaleString()}${item.next_attempt_at ? `<br>Deferred until ${new Date(item.next_attempt_at*1000).toLocaleString()}` : ""}${item.dispatch_error ? `<br>${safe(item.dispatch_error)}` : item.last_error ? `<br>${safe(item.last_error)}` : ""}</small><div>${["queued","forwarding"].includes(item.state)?`<button data-relay-action="pause" data-envelope="${safe(item.envelope_id)}">Pause</button>`:""}${item.state==="paused"?`<button data-relay-action="resume" data-envelope="${safe(item.envelope_id)}">Resume</button>`:""}${!["expired","purged"].includes(item.state)&&["failed","pending"].includes(item.dispatch_status)?`<button data-relay-action="retry" data-envelope="${safe(item.envelope_id)}">Retry local dispatch</button>`:""}${item.state!=="purged"?`<button class="danger" data-relay-action="purge" data-envelope="${safe(item.envelope_id)}">Purge payload</button>`:""}</div>${relayHistory(item)}</article>`).join("") || `<p class="ui-empty empty">No relay custody history.</p>`;
  ...
  document.querySelectorAll("[data-origin-replace],[data-origin-reject-candidate]").forEach(button => button.addEventListener("click", async () => { const origin=button.dataset.originReplace||button.dataset.originRejectCandidate,state=button.dataset.originReplace?"replace":"reject_candidate",verb=state==="replace"?"replace the current pin with":"reject";if(!await window.OutpostUI.confirm({title:`${state==="replace"?"Use":"Reject"} origin-key candidate?`,message:`This will ${verb} ${button.dataset.fingerprint}. Verify the origin and fingerprint out of band first.`,confirmLabel:state==="replace"?"Use candidate":"Reject candidate",danger:true}))return;await api(`/api/v1/federation/relay/origins/${encodeURIComponent(origin)}`,{method:"PATCH",body:JSON.stringify({state,fingerprint:button.dataset.fingerprint})});await refreshStoreForward(); }));
  document.querySelectorAll("[data-origin-forget]").forEach(button => button.addEventListener("click", async () => { if(!await window.OutpostUI.confirm({title:"Forget authoritative origin key?",message:"Future multi-hop envelopes will be quarantined until a direct-origin proof or a new operator decision establishes the key again.",confirmLabel:"Forget pin",danger:true}))return;await api(`/api/v1/federation/relay/origins/${encodeURIComponent(button.dataset.originForget)}`,{method:"PATCH",body:JSON.stringify({state:"forget"})});await refreshStoreForward(); }));
