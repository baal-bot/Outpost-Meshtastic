...
async function refreshStoreForward() {
  ...
  $("relay-summary").innerHTML = `<span><b>${active}</b> active custody items</span><span><b>${result.summary.stored_bytes}</b> stored bytes</span><span>${result.identity?.fingerprint?`Local key ${safe(result.identity.fingerprint.slice(0,16))}…`:"Local key status unavailable"}</span>`;
