...
async function refreshSyncStatus() {
  ...
  const items = (await response.json()).items;
  target.innerHTML = items.map(peer => `<article class="sync-row"><div><strong>${safe(peer.node_name || peer.mesh_id)}</strong><code>${safe(peer.mesh_id)} · ${safe(peer.state)}</code></div><div><b>${peer.pending_items}</b><span>Pending</span></div><div><b>${peer.imported_items}</b><span>Imported</span></div><div><b>${peer.rejected_items}</b><span>Rejected</span></div><p>${peer.last_sync_at ? `Last sync ${age(peer.last_sync_at)}` : "No completed cycle"}<br>${peer.cursors.length} receive cursor${peer.cursors.length === 1 ? "" : "s"}</p></article>`).join("") || `<p class="empty">No paired sync activity yet.</p>`;
}
...
async function refresh() {
  ...
  $("peer-list").innerHTML = items.map(peer => `<article class="peer-card"><div class="peer-head"><div><strong>${safe(peer.node_name || "Unnamed Outpost")}</strong><br><code>${safe(peer.mesh_id)}</code></div><span class="chip ${safe(peer.state)}">${safe(peer.state)}</span></div><div class="peer-meta">${peer.discovery_transports.map(item => `<span class="chip">${safe(item)}</span>`).join("")}${Object.entries(peer.capabilities).filter(([, enabled]) => enabled).map(([name]) => `<span class="chip">${safe(name)}</span>`).join("")}</div><p>Last heard ${age(peer.last_seen_at)} · Protocol ${safe(peer.protocol_version)}</p>${peer.state === "pairing" ? `<div class="pair-box" data-code-for="${safe(peer.mesh_id)}">Waiting for key exchange…</div>` : ""}<div class="peer-actions">${peer.state === "pending" ? `<button data-pair="${safe(peer.mesh_id)}">Pair securely</button>` : ""}${peer.state === "paused" ? `<button data-state="pending" data-id="${safe(peer.mesh_id)}">Resume review</button>` : peer.state !== "active" ? `<button data-state="paused" data-id="${safe(peer.mesh_id)}">Pause</button>` : ""}<button class="danger" data-state="rejected" data-id="${safe(peer.mesh_id)}">Reject</button></div></article>`).join("") || `<p class="empty">No Outposts match this view. Discovery announcements are intentionally infrequent to conserve airtime.</p>`;
  ...
  for (const box of document.querySelectorAll("[data-code-for]")) {
    ...
    if (codeResponse.ok) { const code = (await codeResponse.json()).confirmation_code; box.innerHTML = `<strong>Verify on both Outposts: ${safe(code)}</strong><span>Only approve if both operators see this exact code.</span><div><input inputmode="numeric" maxlength="6" value="${safe(code)}" aria-label="Confirmation code"><button data-approve="${safe(id)}">Approve pairing</button></div>`; }
  }
