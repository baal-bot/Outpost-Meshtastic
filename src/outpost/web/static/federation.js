...
async function refreshStoreForward() {
  ...
  $("relay-summary").innerHTML = `<span><b>${active}</b> active custody items</span><span><b>${result.summary.counts.rejected||0}</b> terminal failures</span><span><b>${result.summary.stored_bytes}</b> stored bytes</span><span>${result.identity?.fingerprint?`Local key ${safe(result.identity.fingerprint.slice(0,16))}…`:"Local key status unavailable"}</span>`;
  ...
  queue.innerHTML = result.queue.map(item => `<article class="relay-item"><div><strong>${safe(item.scope)} · ${item.state==="rejected"?"terminal failure":safe(item.state)}</strong><code>${safe(item.envelope_id)}</code></div><p>${safe(item.origin_node)} → ${safe(item.destination_node)}<br>Route: ${item.route.map(safe).join(" → ")}${item.last_path ? `<br>Selected ${safe(item.last_path)} path → ${safe(item.next_hop_mesh_id)}` : ""}${item.received_transport ? `<br>Received by ${safe(item.received_transport.toUpperCase())}` : ""}<br>${safe(item.attempts)} forwarding attempt(s)</p><small>Expires ${new Date(item.expires_at*1000).toLocaleString()}${item.next_attempt_at ? `<br>Deferred until ${new Date(item.next_attempt_at*1000).toLocaleString()}` : ""}${item.last_error ? `<br>${safe(item.last_error)}` : ""}</small><div>${["queued","forwarding"].includes(item.state)?`<button data-relay-action="pause" data-envelope="${safe(item.envelope_id)}">Pause</button>`:""}${item.state==="paused"?`<button data-relay-action="resume" data-envelope="${safe(item.envelope_id)}">Resume</button>`:""}${item.state!=="purged"?`<button class="danger" data-relay-action="purge" data-envelope="${safe(item.envelope_id)}">Purge payload</button>`:""}</div>${relayHistory(item)}</article>`).join("") || `<p class="ui-empty empty">No relay custody history.</p>`;
