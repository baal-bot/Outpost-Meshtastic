...
async function refresh() {
  ...
  $("peer-list").innerHTML = items.map(peer => `<article class="peer-card"><div class="peer-head"><div><strong>${safe(peer.node_name || "Unnamed Outpost")}</strong><br><code>${safe(peer.mesh_id)}</code></div><span class="chip ${safe(peer.state)}">${safe(peer.state)}</span></div><div class="peer-meta">${peer.discovery_transports.map(item => `<span class="chip">${safe(item)}</span>`).join("")}${Object.entries(peer.capabilities).filter(([, enabled]) => enabled).map(([name]) => `<span class="chip">${safe(name)}</span>`).join("")}</div><p>Last heard ${age(peer.last_seen_at)} · Protocol ${safe(peer.protocol_version)}</p>${peer.state === "pairing" ? `<div class="pair-box" data-code-for="${safe(peer.mesh_id)}">Waiting for key exchange…</div>` : ""}<div class="peer-actions">${peer.state === "pending" ? `<button data-pair="${safe(peer.mesh_id)}">Pair securely</button>` : ""}${peer.state === "paused" ? `<button data-state="pending" data-id="${safe(peer.mesh_id)}">Resume review</button>` : peer.state !== "active" ? `<button data-state="paused" data-id="${safe(peer.mesh_id)}">Pause</button>` : ""}${peer.state === "active" ? `<button class="danger" data-state="pending" data-id="${safe(peer.mesh_id)}">Unpair</button>` : `<button class="danger" data-state="rejected" data-id="${safe(peer.mesh_id)}">Reject</button>`}</div></article>`).join("") || `<p class="empty">No Outposts match this view. Discovery announcements are intentionally infrequent to conserve airtime.</p>`;
