...
def specs(service: BBSService, self_delete_minutes: int = 30) -> list[CommandSpec]:
    ...
    return [
        CommandSpec("BOARDS", ("BL",), help_short="BOARDS · list boards", handler=boards, **base),
        CommandSpec(
            "BOARD", ("B",), help_short="BOARD <name> · recent threads", handler=board, **base
        ),
        CommandSpec("POST", ("P",), help_short="POST <board> <text>", handler=post, **base),
        CommandSpec("READ", ("R",), help_short="READ <n> · open thread", handler=read, **base),
        CommandSpec("REPLY", ("RE",), help_short="RE [thread] <text>", handler=reply, **base),
        CommandSpec("SEARCH", ("S",), help_short="SEARCH <terms>", handler=search, **base),
        CommandSpec("NEW", ("N",), help_short="NEW · unread digest", handler=new, **base),
        CommandSpec("MORE", ("+", "M+"), help_short="MORE · next page", handler=more, **base),
        CommandSpec("SUB", (), help_short="SUB <board> [cadence]", handler=subscribe, **base),
        CommandSpec("UNSUB", (), help_short="UNSUB <board>", handler=unsubscribe, **base),
        CommandSpec("RMPOST", (), help_short="RMPOST <ref>", handler=remove_post, **base),
    ]
